...
10.部署自检：配置缺失时图床页（/imgbed）会直接指出缺哪个环境变量或绑定、去哪里补，而不是等到第一次上传才失败
...
13.**局域网传输（LocalSend 兼容）**：访问 `/localsend`，可向同一局域网内运行 LocalSend 的设备发送文件（协议 v2：register / prepare-upload / upload，支持 PIN 与取消），也支持通过「链接分享」从设备接收文件；浏览器与浏览器互传可继续使用 `/snap` 的 WebRTC P2P 快传

## 页面结构

| 路由 | 说明 |
| ----------- | ----------- |
| `/` | 产品介绍首页：图床 / 网盘 / 快传三大核心功能入口卡片、平台特性说明与更多工具导航 |
| `/imgbed` | 图床上传页（原旧版首页）：批量 / 拖拽 / 粘贴上传，多格式链接一键复制 |
| `/netdisk` | 网盘文件管理（需绑定 R2） |
| `/snap` | WebRTC P2P 快传（房间码互传，不经过服务器存储） |
| `/localsend` | 局域网传输页，兼容 LocalSend 协议 v2 |
| `/admin` | 管理后台 |

### 局域网传输（LocalSend）使用说明

- **发送到设备**：在 `/localsend` 输入对方设备地址（如 `192.168.1.8`，默认端口 `53317`）或按网段扫描，选中设备与文件即可发送；对方确认后文件经局域网直传。对方若开启 PIN，页面会提示输入。
- **从设备接收**：在 LocalSend 应用中选择「通过链接分享」，在页面「从设备接收」中输入设备地址即可列出并下载文件。
- **浏览器限制须知**：浏览器无法监听端口/组播，故本页只能主动连接设备、不会被对方「发现」；LocalSend 默认的 HTTPS 自签名证书不被浏览器信任，需在对方设备「设置 → 网络」中关闭加密（改用 HTTP）；若本站以 HTTPS 访问，浏览器会拦截发往局域网 HTTP 地址的请求（混合内容），建议在内网通过 HTTP 访问本站。页面内置 `/api/lan/relay` 同源中继（仅允许私有网段目标），内网自托管时体验最佳；公网部署时中继无法触达内网，浏览器间互传请改用「快传」。
...
## 更新日志
2026 年 8 月 2 日--首页重构与局域网传输（LocalSend 兼容）

- **首页重构为产品介绍页**：`/` 现在以卡片式布局集中展示图床、网盘、快传三大核心功能入口，附平台特性说明与「更多工具」导航（局域网传输、管理后台、旧版界面）；原上传功能迁移至 `/imgbed`
- **新增局域网传输** `/localsend`：兼容 LocalSend 协议 v2（register / prepare-upload / upload / prepare-download / download），支持手动添加设备或按网段扫描、PIN 验证、传输取消、逐文件进度；配套新增同源中继端点 `/api/lan/relay`（仅允许私有网段目标，防开放代理滥用）
- 各页面导航统一互联（首页 / 图床 / 网盘 / 快传 / 局域网 / 后台），网盘页品牌区改为可点击回首页
- 端到端测试同步更新：首页改为校验三大功能入口，上传流程改在 `/imgbed` 执行；新增局域网收发流程的 Playwright 用例与 relay 单元测试
